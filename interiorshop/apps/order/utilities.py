...
def checkout(request, first_name, last_name, email, address, zipcode, place, phone, amount):
    ...
    for item in Cart(request):
        ...
        ordr=OrderItem(order=order, product=item['product'], vendor=item['product'].vendor, price=item['product'].price, quantity=item['quantity'])
        ordr.save()
...
def notify_vendor(order):
    ...
    from_email ='revadibrunda@gmail.com'
